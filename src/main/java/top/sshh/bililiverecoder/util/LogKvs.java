...
public final class LogKvs {
    ...
    static {
        ...
        EVENT_ZH.put("History.BatchForceArchive.ItemFailed", "批量强制归档：单项处理失败");
        EVENT_ZH.put("History.BatchUpload.ItemFailed", "批量上传设置：单项处理失败");
        EVENT_ZH.put("History.GiftReplyLineCount.QueryFailed", "礼物回复：查询可发送行数失败");
        EVENT_ZH.put("History.MsgRetry.Done", "历史弹幕重试：已重新加入发送队列");
        EVENT_ZH.put("History.MsgRetry.NoDispatchableFailed", "历史弹幕重试：没有可重新调度的失败记录");
        EVENT_ZH.put("History.MsgRetry.None", "历史弹幕重试：没有可重试的弹幕");
        EVENT_ZH.put("HistoryStateReconcile.Failed", "历史状态校准：处理失败");
        EVENT_ZH.put("HistoryStateReconcile.Fixed", "历史状态校准：已修正稿件状态");
        ...
        EVENT_ZH.put("Publish.TimestampJump.AllPartsGiveUp", "投稿：时间戳跳变导致所有分P放弃投稿");
        EVENT_ZH.put("Publish.TimestampJump.RetryWithoutParts", "投稿：移除时间戳异常分P后重试");
        EVENT_ZH.put("Publish.TimestampJump.Unresolved", "投稿：时间戳跳变无法定位异常分P，停止投稿");
        ...
        EVENT_ZH.put("FileOpen.IgnoreCompletedStaleSession", "文件打开事件：已完成的旧会话到达，忽略");
        EVENT_ZH.put("PartReconcile.Closed", "分P校准：已关闭过期录制分P");
        EVENT_ZH.put("PartReconcile.Done", "分P校准：本轮处理完成");
        EVENT_ZH.put("PartReconcile.HistoryClosed", "分P校准：已关闭没有进行中分P的稿件");
        EVENT_ZH.put("PartReconcile.Skip.InvalidFileTime", "分P校准：文件修改时间早于录制开始时间，跳过本次校准");
        EVENT_ZH.put("RecordStarted.IgnoreCompletedStaleSession", "录制开始：已完成的旧会话到达，忽略");
        ...
        EVENT_ZH.put("StorageRoot.WorkPath.NotConfigured", "存储目录：尚未配置工作目录");
        EVENT_ZH.put("StorageRoot.WorkPathChange.PendingReview", "工作目录变更：等待确认和后台校验");
        ...
        EVENT_ZH.put("LiveMsgSendSync.Visibility.High.SwitchPublic.Response", "SC/上舰弹幕：切换公开响应");
        ...
        EVENT_ZH.put("LiveMsgSendSync.Visibility.High.SwitchPrivate.Response", "SC/上舰弹幕：切回仅自己可见响应");
        ...
        EVENT_ZH.put("VideoSync.AuditDetail.LoadFailed", "稿件状态同步：加载审核详情失败");
        EVENT_ZH.put("VideoSync.ManualRefresh.KeepPendingAfterRecentEdit", "稿件状态刷新：近期编辑后暂时保持审核中状态");
        ...
        EVENT_ZH.put("RoomLiveEvent.Parse.IssueDetected", "直播事件解析：发现需要处理的 XML 问题");
        EVENT_ZH.put("RoomLiveEvent.Parse.IssueMigration", "直播事件解析：已迁移旧 XML 问题状态");
        EVENT_ZH.put("RoomLiveEvent.Parse.IssueSummary", "直播事件解析：已汇总跳过待处理 XML");
        EVENT_ZH.put("RoomLiveEvent.Parse.RecheckFailed", "直播事件解析：重新检查任务失败");
        ...
        EVENT_ZH.put("Stats.Room.Delete.Success", "统计数据：房间相关数据删除完成");
        ...
        EVENT_ZH.put("Room.Delete.Error", "房间删除：处理失败");
        EVENT_ZH.put("Room.Delete.Success", "房间删除：处理完成");
        ...
        EVENT_ZH.put("Notify.Event.Dispatch.Failed", "通知事件：发送分发失败");
        EVENT_ZH.put("Notify.Event.Submit.Failed", "通知事件：提交异步任务失败");
        EVENT_ZH.put("Notify.Publish.SummaryStatsRefreshFailed", "投稿通知：刷新统计摘要失败");
