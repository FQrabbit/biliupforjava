...
(function(window) {
    ...
    window.HistoryPagePreviewMethods = {
        ...
        canUseGlobalPartPreview: function() {
            try {
                return !!(window.parent && window.parent !== window && window.parent.GlobalPartPreviewPlayer && typeof window.parent.GlobalPartPreviewPlayer.open === 'function');
            } catch (e) {
                return false;
            }
        },
        ...
        transferPartPreviewToGlobal: function(collapsed) {
            ...
            var opened = false;
            try {
                opened = window.parent.GlobalPartPreviewPlayer.open(payload) !== false;
            } catch (e) {
                opened = false;
            }
        ...
        preparePartPreview: function() {
            ...
            if (_this.isPreviewCacheReady && !_this.isPreviewTaskActive) {
                _this.$confirm('重新生成会删除当前 MP4 预览缓存，并重新调用 ffmpeg 封装。确认继续吗？', '重新生成预览', {
