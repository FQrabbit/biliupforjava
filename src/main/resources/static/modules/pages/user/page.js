...
return {
    ...
    watch: {
        ...
        dialogFormVisible: function () { this.syncPageModalState(); },
        dialogLoginVisible: function (visible) {
            if (!visible) {
                this.loginRequestToken++;
                this.stopLoginCheck();
            }
            this.syncPageModalState();
        },
        mobileUserActionsVisible: function () { this.syncPageModalState(); },
        brecSyncResultVisible: function () { this.syncPageModalState(); }
    },
    methods: {
        ...
        syncPageModalState: function () {
            this.$emit('page-state', {
                kind: 'modal',
                source: 'user',
                active: !!(this.dialogFormVisible || this.dialogLoginVisible || this.mobileUserActionsVisible || this.brecSyncResultVisible)
            });
        },
        fetchUserList: function () {
            ...
            UserApi.list(function (data) {
                ...
                self.$emit('connection-status', true);
            });
        },
        getLoginImage: function () {
            var self = this;
            self.stopLoginCheck();
            var requestToken = ++self.loginRequestToken;
            self.loginLoading = true;
            self.loginStatus = 'pending';

            UserApi.loginQr(function (data) {
                if (self.componentDestroyed || requestToken !== self.loginRequestToken) return;
        ...
        startLoginCheck: function () {
            var self = this;
            var requestToken = this.loginRequestToken;
            if (this.componentDestroyed || !this.dialogLoginVisible) return;
            self.loginCheckTimer = setInterval(function () {
                if (self.componentDestroyed || requestToken !== self.loginRequestToken || !self.dialogLoginVisible) {
                    self.stopLoginCheck();
                    return;
                }
                ...
                UserApi.loginCheck(self.loginKey, function (data) {
                    if (self.componentDestroyed || requestToken !== self.loginRequestToken || !self.dialogLoginVisible) return;
        ...
        deleteMobileUser: function (item) {
            ...
            self.$pageConfirm('确定移除账号「' + name + '」吗？', '移除账号', {
    ...
    beforeDestroy: function () {
        ...
        this.$emit('page-state', { kind: 'modal', source: 'user', active: false });
    }
};
});
