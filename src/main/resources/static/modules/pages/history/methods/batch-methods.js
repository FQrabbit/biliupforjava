...
(function(window) {
    ...
    window.HistoryPageBatchMethods = {
        ...
        beginBatchDeleteOperation: function() {
            this.batchDeleteRunning = true;
            this.$emit('page-state', {
                kind: 'operation',
                source: 'history-batch-delete',
                active: true,
                message: '批量删除录制历史',
                blockingClose: true
            });
        },
        finishBatchDeleteOperation: function() {
            this.batchDeleteRunning = false;
            if (this.batchDeleteLoading && typeof this.batchDeleteLoading.close === 'function') {
                this.batchDeleteLoading.close();
            }
            this.batchDeleteLoading = null;
            this.$emit('page-state', {
                kind: 'operation',
                source: 'history-batch-delete',
                active: false
        ...
        confirmBatchDelete: function() {
            ...
            this.batchDeleteDialogVisible = false;
            this.beginBatchDeleteOperation();
            try {
                this.batchDeleteLoading = this.$pageLoading({
                    lock: true,
                ...
                this.$message.error('无法启动批量删除界面，请重试');
                return;
            }
            ...
            Promise.all(promises).then(function(results) {
                ...
                if (notDeletedGrouped.length > 0) {
                    _this.$pageAlert(_this.buildNotDeletedFilesHtml(notDeletedGrouped), '部分本地文件未删除', {
                        dangerouslyUseHTMLString: true,
                        confirmButtonText: '知道了',
                        type: 'warning'
                    });
                ...
                _this.finishBatchDeleteOperation();
            });
        },
        ...
        handleBatchVisibility: function(isOnlySelf) {
            ...
            this.$pageConfirm(msg, '批量切换可见性确认', {
                dangerouslyUseHTMLString: true,
        ...
        sleepMs: function(ms) {
            var self = this;
            return new Promise(function(resolve) {
                if (typeof self.scheduleHistoryDeferred === 'function') {
                    self.scheduleHistoryDeferred(resolve, ms);
                } else {
